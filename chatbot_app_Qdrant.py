...
# chatbot_app_Qdrant.py
# Full Streamlit RAG app with Qdrant Cloud + FAISS + Hybrid retrieval,
# OCR for PDFs and images, multi-query expansion, reranking, similarity filtering,
# debug view, index management (select/delete/reset), feedback logging,
# admin dashboard, and reset chat.
            ...
            for img in soup.find_all("img", src=True):
                ...
                try:
                    ...
                    if img_resp.status_code == 200:
                        img_text = extract_text_from_image_bytes(img_resp.content)
                        if img_text.strip():
                            results.append((img_url, img_text))
                except Exception:
                    continue
    ...
    try:
        if uploaded_file.type == "text/plain":
            ...
            if not text.strip():
                ...
                blocks = [pytesseract.image_to_string(img, lang="eng") for img in images]
                text = "\n".join(blocks)

        elif uploaded_file.type.startswith("image/"):
            text = extract_text_from_image_bytes(uploaded_file.read())

        else:
            st.warning(f"⚠️ Unsupported file type: {uploaded_file.type}")
            return uploaded_file.name, None

        text = clean_text(text)
        return uploaded_file.name, text if text else None
...
# =========================
# (continues with Index Management, Retrieval pipeline, Chatbot UI, Admin Dashboard...)
# =========================
# =========================
# Retrievers
# =========================
...
def build_faiss_index(collection_name: str, docs: List[Document], embeddings):
    if not FAISS_AVAILABLE:
        st.sidebar.error("FAISS not available. Install faiss-cpu to enable.")
        return None
    vs = FAISS.from_documents(docs, embeddings)
    idx_path, pkl_path = faiss_paths(collection_name)
    vs.save_local(folder_path=idx_path.parent.as_posix(), index_name="faiss")
    return vs
...
def load_faiss_index(collection_name: str, embeddings):
    ...
    if idx_path.exists() and pkl_path.exists():
        return FAISS.load_local(
            folder_path=idx_path.parent.as_posix(),
...
max_pages = st.sidebar.slider("📄 Max Pages", 1, 30, 5)
...
similarity_threshold = st.sidebar.slider("Similarity threshold", 0.0, 1.0, 0.30, 0.01)
...
if qdrant_collections:
    ...
    if st.sidebar.button("⚠️ Delete ALL Indexes"):
        try:
...
# Delete FAISS
if FAISS_AVAILABLE:
    st.sidebar.subheader("🗑️ Manage FAISS")
    local_idx_dirs = [p for p in INDEX_DIR.glob("*") if p.is_dir()]
    if local_idx_dirs and st.sidebar.button("⚠️ Delete ALL FAISS indexes"):
        for d in local_idx_dirs:
            try:
                shutil.rmtree(d, ignore_errors=True)
            except Exception:
                pass
        st.sidebar.warning("Deleted all local FAISS indexes")
...
if source_mode == "Website":
    ...
    uploaded_files = st.sidebar.file_uploader("📂 Upload PDF/TXT/Image", type=["pdf", "txt", "png", "jpg", "jpeg"], accept_multiple_files=True)
    ...
    if not labeled:
        ...
        if "FAISS" in vector_store_choice and FAISS_AVAILABLE:
            try:
                ...
                st.sidebar.success(f"✅ FAISS index built: {collection_name}")
                built_any = True
            except Exception as e:
...
def retrieve_docs(question: str, top_k: int, threshold: float) -> Tuple[List[Document], float]:
    ...
    if vector_store_choice.startswith("Qdrant"):
        ...
        for q in queries:
            prelim_docs.extend(
                hybrid_retrieve(q, get_qdrant_client(), st.session_state["active_index"], embeddings, k=top_k)
            )
    ...
    ranked = rerank_documents(question, prelim_docs, top_k=max(top_k, 10))
...
if app_mode == "Chatbot":
    st.markdown(
        f"### 💬 Chatbot (Backend: `{vector_store_choice}` — Active Index: `{st.session_state.get('active_index','None')}`)"
    )
        ...
        if debug_mode:
            with st.expander("🔎 Retrieved Chunks (Debug)"):
                ...
                for i, d in enumerate(docs, 1):
                    ...
                    export_data.append(
                        {"rank": i, "source": src, "score": score, "content": d.page_content}
                    )
                st.download_button(
                    "⬇️ Download JSON",
                    data=json.dumps(export_data, indent=2, ensure_ascii=False),
                    file_name="retrieved.json",
